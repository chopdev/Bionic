   ...
   partial class StoreDB
    {
        ...
        string EntryTests = "Select * FROM `EntryTest`";
       ...
       public MySqlConnection Connection;
        ...
        public List<EntryTest> AllEntrytests()
        {
            List<EntryTest> Alltests = new List<EntryTest>();
            MySqlCommand cmd = Connection.CreateCommand();
            cmd.CommandText = EntryTests;
            try
            {
                Connection.Open();
                ...
                while (data.Read())
                {
                    if (!Int32.TryParse(data["entrytestid"].ToString(), out et_id)) continue;
                    Alltests.Add(new EntryTest(et_id, data["name"].ToString()));
                }
            }
            catch
            {
